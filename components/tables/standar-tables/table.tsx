...
async function deleteData(id: UUID, onDeleted?: () => void) {
  ...
  const response_api = await fetch(`/api/standar?standarId=${id}`, {
    method: 'DELETE'
  });
...
export default function Table(props: TableProps) {
  ...
  useEffect(() => {
    ...
    let url = filterUrl ? new URL(`${window.location.origin}/api/standar?${filterUrl}`) : new URL(`${window.location.origin}/api/standar?search=${props.idreg}`)
  ...
  }, [pageParams, pathname, router, search, props.idreg])
